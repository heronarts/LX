...
public abstract class LXPeriodicModulator extends LXModulator {
  ...
  protected final double computeValue(double deltaMs) {
    ...
    if (this.tempoSync.isOn()) {
      if (this.tempoLock.isOn()) {
        double basis = getLX().tempo.compositeBasis() * this.tempoMultiplier.getEnum().multiplier;
        this.basis = basis % 1.;
        ...
        this.basis += deltaMs / getLX().tempo.period.getValue() * this.tempoMultiplier.getEnum().multiplier;
      }
    } else if (periodv == 0) {
      this.basis = 1;
    } else {
