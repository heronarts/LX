...
public class Noise {
  ...
  private static final byte[] stb__perlin_randtab_grad_idx =
  {
  ...
  private static float stb__perlin_lerp(float a, float b, float t)
  {
    return a + (b-a) * t;
  }

  private static int stb__perlin_fastfloor(float a)
  {
    int ai = (int) a;
    return (a < ai) ? ai-1 : ai;
  }

  private static final float[][] stb__perlin_grad_basis =
  {
    {  1, 1, 0 },
    { -1, 1, 0 },
    {  1,-1, 0 },
  ...
  private static float stb__perlin_grad(int grad_idx, float x, float y, float z)
  {
    float[] grad = stb__perlin_grad_basis[grad_idx];
    return grad[0]*x + grad[1]*y + grad[2]*z;
  }

  private static float stb__perlin_ease(float a)
  {
    return (((a*6-15)*a + 10) * a * a * a);
  }

  private static float stb_perlin_noise3_internal(float x, float y, float z, int x_wrap, int y_wrap, int z_wrap, int seed)
  {
    ...
    int x_mask = (x_wrap-1) & 0xff;
    int y_mask = (y_wrap-1) & 0xff;
    int z_mask = (z_wrap-1) & 0xff;
